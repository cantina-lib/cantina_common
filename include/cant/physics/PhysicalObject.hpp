...
class PhysicalObject : public Positionable<Len_T, dim>
{
   public:
    /** -- methods -- **/
    virtual void
      translate(typename Positionable<Len_T, dim>::Vector const & vec)
      = 0;
};
...
class PhysicalObjectTrait : public PhysicalObject<Len_T, dim>
{
   ...
   public:
    ...
    void
      translate(typename PhysicalState<Len_T, dim>::Vector const & vec) override;

    // implemented from Positionable
    void
      setPosition(typename PhysicalState<Len_T, dim>::Position position) override;
    CANT_NODISCARD typename PhysicalState<Len_T, dim>::Position const &
