...
CANTINA_MATHS_NAMESPACE_BEGIN
    ...
    Rectangle<Dim_T, dim>::
    Rectangle(Rectangle::Point centre, Rectangle::Point halfDims)
        : m_centre(std::move(centre)),
        m_halfDims(std::move(halfDims))
    { }
    ...
    intersects(Rectangle const& other) const
    {
        ...
        {
            if (
                std::abs(this->m_centre.get(i) - other.m_centre.get(i))
                > this->m_halfDims.get(i) + other.m_halfDims.get(i)
                )
            {
                return false;
            }
